...
//             setEditStatus('PENDING');
//           }}
//           className="bg-[#800020] hover:bg-[#800020]/70 text-orange-300 px-6 py-2 rounded font-bold shadow-lg border border-orange-400 transition duration-200"
//         >
//           + Tambah Minuman
//         </button>
...
//           <div className="rrelative bg-gradient-to-br from-red-500 p-8 rounded-2xl max-w-xl w-full shadow-2xl border-4 border-orange-600 animate-fade-in overflow-hidden">
...
export default function DrinkPage() {
  ...
  try {
    ...
    const res = await fetch(
      `/api/products?category=drink&search=${encodeURIComponent(search)}&page=${currentPage}&limit=${itemsPerPage}`
    );
    if (!res.ok) {
      const { error } = await res.json();
      throw new Error(error || 'Gagal memuat produk');
...
//           <Search placeholder="Cari minuman..."/>
//         </div>
//       </div>
