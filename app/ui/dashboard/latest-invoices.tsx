...
export default function LatestInvoices() {
  ...
  if (isLoading) {
    return <p className="text-orange-500 animate-pulse text-lg">Summoning invoices from the beyond...</p>;
  }

  if (error) {
    return <p className="text-red-700 font-bold text-lg">🔮 A curse has been placed on your data: {error}</p>;
  }

  if (!latestInvoices.length) {
    return <p className="text-gray-400 text-lg">The graveyard is empty. No invoices found.</p>;
  }

  return (
    <div className="flex w-full flex-col md:col-span-4">
      <h2 className={`${lacquer.className} mb-4 text-xl md:text-2xl text-orange-600 font-bold`}></h2>
      ...
      <div className="flex grow flex-col justify-between rounded-xl bg-gray-950 p-4 border-2 border-orange-700 shadow-[0_0_20px_#fb923c33]">
        ...
        <div className="flex items-center justify-center pb-2 pt-6">
          {/* <p className="text-sm italic text-orange-400 opacity-80">
            Updated under the blood moon 🌕
          </p> */}
        </div>
      </div>
    </div>
